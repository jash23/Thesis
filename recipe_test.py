...
model.pc = Set()  # set of priority cases
...
model.sparse_pr1 = Set(model.pc, dimen=2)
model.sparse_pr2 = Set(model.pc, dimen=2)
...
model.slack = Var(model.pc, within=NonNegativeReals)
model.dv = Var(model.pc, within=Binary)
...
model.pr1 = Param(model.K, model.P, model.pc, default=0)
model.pr2 = Param(model.K, model.P, model.pc, default=0)
...
def objective_rule(model):
    ...
    return sum(model.a[k] * model.X[k, p] if model.a[k] != 0 else 0 for k,p in model.j.sparse_iterkeys()) + sum(model.a[k] * model.i[k] * model.I[k] for k in model.a.sparse_iterkeys())
...
def demand_end(model, k):
    if k not in model.sparse_j:
        ...
        return sum(model.j[k, p]*model.X[k, p] for p in model.sparse_j[k]) <= max(model.l[k]*model.d[k], 0)
...
def mass_balance(model, k):
    ...
    return sum(model.e[k, p]*model.X[k, p] for p in model.sparse_e[k]) == sum(model.h[k, p]*(-model.X[k, p]) for p in model.sparse_h[k])+(model.i[k]*model.I[k])
model.mass_balance = Constraint(model.T, rule=mass_balance)

# def detect_sparsity_pr1(model):
#     for k,p,priority in model.pr1.sparse_iterkeys():
#         model.sparse_pr1[priority].add((k,p))
# model.detect_sparcity_pr1 = BuildAction(rule=detect_sparsity_pr1)

# def priority_check1(model, priority):
#     return (model.pr1[4,'B_40000000_0100',priority] * model.X[4,'B_40000000_0100']/model.u['B_40000000_0100']) + model.slack[priority] == 1
# model.priority_check1 = Constraint(model.pc, rule=priority_check1)
#
# def decision2(model, priority):
#     return model.slack[priority] >= 1 - model.dv[priority]
# model.decision2 = Constraint(model.pc, rule=decision2)
#
# # def detect_sparsity_pr2(model):
# #     for k,p,pc in model.pr2.sparse_iterkeys():
# #         model.sparse_pr2[pc].add((k,p))
# # model.detect_sparcity_pr2 = BuildAction(rule=detect_sparsity_pr2)
#
# def priority_check2(model, priority):
#     return (model.pr2[4, 'A_40000000_0200', priority] * model.dv[priority] * model.X[4, 'A_40000000_0200']) == 0
# model.priority_check2 = Constraint(model.pc, rule=priority_check2)

def priority_check1(model):
    return model.X[4,'B_40000000_0100']/model.u['B_40000000_0100'] + model.slack[1] == 1
model.priority_check1 = Constraint(rule=priority_check1)

def decision2(model):
    return model.slack[1] >= 1 - model.dv[1]
model.decision2 = Constraint(rule=decision2)

def priority_check2(model):
    return model.dv[1] * model.X[4, 'A_40000000_0200'] == 0
model.priority_check2 = Constraint(rule=priority_check2)
...
# instance = model.create_instance("recipe_test.dat")
